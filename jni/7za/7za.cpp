...
#include "7za.h"
#include "7za_log.h"
#include "com_droid7z_Droid7z.h"

//#define  ARGC 9
//static const char *test_args [ARGC+1] = {
//    "7za",
//    "e",
//    "-o/data/data/com.droid7z/libs",
//    "/data/data/com.droid7z/libs/volar.lib",
//    "x86/libffmpeg.so",
//    "x86/libomx-9.so",
//    "x86/libomx-11.so",
//    "x86/libomx-14.so",
...
JNIEXPORT jint JNICALL Java_com_droid7z_Droid7z_uncompress(JNIEnv *env, jobject clazz, 
		jstring jdest, jstring jarchive, jobjectArray jfiles)
{
	int i;
	jobject tmp_file;
	jsize num_files = env->GetArrayLength(jfiles);

	// Convert params to char*
	const char *archive = env->GetStringUTFChars(jarchive, JNI_FALSE);
	const char *tmp_dest = env->GetStringUTFChars(jdest, JNI_FALSE);
	char dest[100];
	strcpy(dest, "-o");
	strcat(dest, tmp_dest);


	// Initialize args
	int argc = num_files + 4;
	char const** my_args = new char const *[argc+1];
	my_args[0] = "7za";
	my_args[1] = "e";
	my_args[2] = dest;
	my_args[3] = archive;
	for(i=0; i<num_files; i++) {
		tmp_file = env->GetObjectArrayElement(jfiles, i);
		my_args[4+i] = env->GetStringUTFChars((jstring) tmp_file, JNI_FALSE);
	}
	my_args[num_files + 4] = 0;

	LOG_D("Starting uncompress...");
	jint ret = run_7za(argc, my_args);
	LOG_D("Completed uncompress...");

	// Free copies of jni vars
	env->ReleaseStringUTFChars(jdest, tmp_dest);
	env->ReleaseStringUTFChars(jarchive, archive);
	for(i=0; i<num_files; i++) {
		tmp_file = env->GetObjectArrayElement(jfiles, i);
		env->ReleaseStringUTFChars((jstring) tmp_file, my_args[4+i]);
	}
	return ret;
}
...
stdStream << "  ";
		...
		else if (isExtractGroupCommand || options.Command.CommandType == NCommandType::kList)
		{
			if (isExtractGroupCommand)
			{
        		...
        		LOG_I("before...DecompressArchives!!!");
        		...
        		LOG_I("after...DecompressArchives!!!");
	...
	{
		LOG_E("%s %d", kExceptionErrorMessage, NExitCode::kUserBreak);
		return (NExitCode::kUserBreak);
	}
	catch(const CArchiveCommandLineException &e)
	{
		LOG_E("%s fatal", kExceptionErrorMessage);
		(*g_StdStream) << kExceptionErrorMessage << e << endl;
		return (NExitCode::kUserError);
	}
	catch(const CSystemException &systemError)
	{
		LOG_E("%s %s", kExceptionErrorMessage, systemError);
	...
	{
		LOG_E("%s...", kExceptionErrorMessage);
	...
	{
		LOG_E("%s %s", kExceptionErrorMessage, s);
		(*g_StdStream) << kExceptionErrorMessage << s << endl;
		return (NExitCode::kFatalError);
	}
	catch(int t)
	{
		LOG_E("%s %d", kExceptionErrorMessage, t);
